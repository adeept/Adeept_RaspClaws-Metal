...
class CVThread(threading.Thread):
    ...
    def findLineCtrl(self, posInput, setCenter):
        if posInput:
            if posInput > 480:
                CVThread.robot.command_input('right')
                print('right')
            elif posInput <180:
                CVThread.robot.command_input('left') 
                print('left')
            else:
                CVThread.robot.command_input('forward')
                print('forward')
    ...
    def findColor(self, frame_image):
        ...
        if len(cnts) > 0:
            ...
            print(f"self.box_x: {self.box_x}  self.box_y: {self.box_y}")
            M = cv2.moments(c)
            X = int(self.box_x)
            Y = int(self.box_y)
            error_X = 320 - X
            ...
            if abs(error_X) > CVThread.tor:
                if error_X > 0:
                    CVThread.robot.adjust_angle_track_color(12, 1)
                else:
                    CVThread.robot.adjust_angle_track_color(12, -1)
                
            if abs(error_Y) > CVThread.tor:
                if error_Y > 0:
                    CVThread.robot.adjust_angle_track_color(13, -1)
                else:
                    CVThread.robot.adjust_angle_track_color(13, 1)
        else:
            self.findColorDetection = 0
    ...
    def move_stop(self):
        CVThread.robot.command_input('stand')
